...
namespace _04.MOBAChallenger
{
    class MOBAChallenger
    {
        ...
        {
            foreach (KeyValuePair<string, int> position1 in players[player1].infoPlayer)
            {
                foreach (KeyValuePair<string, int> position2 in players[player2].infoPlayer)
                {
    ...
    class PlayerPosition
    {
        ...
        {
            infoPlayer = new Dictionary<string, int>();
        }
        public void AddPosition(string position, int skill)
        {
            if (!infoPlayer.ContainsKey(position))
            {
                infoPlayer.Add(position, skill);
                TotalSkill += skill;
            }
            else if (infoPlayer.ContainsKey(position) &&
                infoPlayer[position] < skill)
            {
                TotalSkill -= infoPlayer[position];
                TotalSkill += skill;
                infoPlayer[position] = skill;
            }
        }
